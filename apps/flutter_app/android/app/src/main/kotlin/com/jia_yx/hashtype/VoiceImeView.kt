...
class VoiceImeView(context: Context) :
        LinearLayout(
                ContextThemeWrapper(
                        context,
    ...
    init {
        ...
        // Neutral keyboard background color (Gboard-like)
        setBackgroundColor(Color.parseColor("#26262651"))
        ...
        tvStatus =
                MaterialTextView(getContext()).apply {
        ...
        btnMic =
                FloatingActionButton(getContext()).apply {
                    setImageResource(android.R.drawable.ic_btn_speak_now)
                    // Increased FAB size for better accessibility
                    val btnSize = (96 * density).toInt()
                    layoutParams = LayoutParams(btnSize, btnSize).apply { gravity = Gravity.CENTER }
                    customSize = btnSize

                    // Set initial color (Material 3 Primary)
                    backgroundTintList =
                            android.content.res.ColorStateList.valueOf(Color.parseColor("#D0BCFF"))
                }
        ...
        val colorHex =
                when (state) {
                    is VoiceImeViewModel.ImeState.Recording ->
                            "#F2B8B5" // M3 Error Container (Pinkish Red)
