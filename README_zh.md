...
- **极简权限，隐私至上：** Hashtype **不需要** 任何危险的系统辅助功能（Accessibility）权限，也不需要额外的系统控制权。与市面上许多需要辅助功能权限来读取屏幕内容的 AI 输入工具不同，Hashtype 仅需要麦克风权限来录制语音。这意味着您的屏幕内容、密码等敏感信息绝不会被读取，隐私得到完全保障。
- **直接通信：** 没有中间服务器。应用直接与你指定的 API 接口通信，确保数据链路安全。
